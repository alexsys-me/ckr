...
let burgerMenu = document.querySelector('.left-menu-wp')
let burger = document.querySelector('.header__burger')
let leftMenuClose = document.querySelector('.left-menu__close')
if(burger){
...
if(ctx){
    ...
    // Данные для графика
    const data = {
        labels: ['11.07', '11.08', '11.09', '11.10', '11.11', '11.12', '11.13', '11.14', '11.15', '11.16', '11.17'],
        datasets: [{
            label: 'My Dataset',
            data: [16, 28, 4, 48, 32, 16, 28,4,48,32,38],
            backgroundColor: ['#e6b911'],
            borderRadius:  window.innerWidth > 600 ? 10 : 5,
            borderWidth: 1
        }]
    };
    ...
    const config = {
        ...
        options: {
            scales: {
                x: {
                    grid: {
                        display: false      // ❌ убрали ВЕРТИКАЛЬНЫЕ линии
                    }
                },
...
if(ctx2){
    ...
    const config = {
        ...
        data: data,
        options: {
            cutout: '80%',       // чем больше — тем тоньше кольцо
            plugins: {},

            elements: {
                arc: {
                    borderWidth: 1  // тонкие линии между сегментами
                }
            }
        }
    };
    const myBarChart2 = new Chart(ctx2, config);
}
...
if(ctx3){
    ...
    const labels =  ['окт. 24', 'ноя. 24', 'дек. 24', 'янв. 25', 'фев. 25', 'мар. 25', 'апр. 25'];
    const data = {
        labels: labels,
        datasets: [{
            label: 'My First Dataset',
    ...
    const myBarChart2 = new Chart(ctx3, config);
}
...
document.addEventListener("click", function(e) {
    ...
    if (!e.target.closest(".dropdown")) {
        document.querySelectorAll(".dropdown._open")
            .forEach(dd => dd.classList.remove("_open"));
        return;
    }
    ...
    if (item) {
        ...
        // показать предыдущий выбранный
        dropdown.querySelectorAll(".dropdown__item.hidden")
            .forEach(old => old.classList.remove("hidden"));
...
document.querySelectorAll('.dropdown').forEach(dropdown => {
    const selected = dropdown.querySelector('.dropdown__item[data-selected]');
    const current = dropdown.querySelector('.dropdown__current ');

    if (selected && current) {
        current.querySelector('span').textContent = selected.textContent;
...
document.addEventListener('click', function (e) {
    const btn = e.target.closest('.new-doc__add');
    if (!btn) return;

    const wrapper = btn.closest('.new-doc__service');

    // создаём новую строку
    const line = document.createElement('div');
    line.className = 'new-doc__line';

    line.innerHTML = `
    <input class="_sablon" type="text" placeholder="Наименование">
    <input class="_sablon" type="text" placeholder="Кол-во">
    <input class="_sablon" type="text" placeholder="Цена">
    <input class="_sablon" type="text" placeholder="Сумма">
  `;

    // вставляем ПЕРЕД кнопками
    wrapper.insertBefore(line, btn);
});


let menuRighEls = document.querySelectorAll('.menu-right__el ');
for (let index = 0; index < menuRighEls.length; index++) {
    const element = menuRighEls[index];
    element.addEventListener('click', function(){
        element.classList.toggle('_active')
    })
}
...
if(viewing){
    function calkHeightViewing(){
        let hh = document.querySelector('.header').offsetHeight
        let partnerTistoryTop = document.querySelector('.partner-history__top').offsetHeight
        viewing.style.setProperty("--height", hh+partnerTistoryTop+'px');
    }
    window.addEventListener('resize', calkHeightViewing())
    window.addEventListener('DOMContentLoaded', calkHeightViewing())
}

let specialistCloseChat = document.querySelector('.specialist__close-chat');
let specialistRight = document.querySelector('.specialist__right');
if(specialistCloseChat){
    specialistCloseChat.addEventListener('click', function(){
        specialistRight.classList.remove('_active')
    })
}

let specialistWpTr = document.querySelectorAll('.specialist__wp tr');

for (let index = 0; index < specialistWpTr.length; index++) {
    const element = specialistWpTr[index];
    element.addEventListener('click', function(){
        specialistRight.classList.add('_active')
    })
}



let dataPopups = document.querySelectorAll('[data-popup]');

for (let index = 0; index < dataPopups.length; index++) {
    const element = dataPopups[index];
    element.addEventListener('click', function(e){
        e.preventDefault()
        document.querySelector(`.${element.dataset.popup}`).classList.add('_active')
    })
}


document.addEventListener('click', function (e) {
    const btn = e.target.closest('._pass-btn');
    if (!btn) return;

    const wrapper = btn.closest('._pass');
    ...
    // чтобы не слетал фокус
    input.focus();
});


// 1️⃣ Жёсткие маски для стран СНГ
const cisdCountries = [
    { name: "Russia", iso2: "ru", dial: "+7", mask: "+7 999 999 9999" },
    { name: "Belarus", iso2: "by", dial: "+375", mask: "+375 99 999 99 99" },
    { name: "Kazakhstan", iso2: "kz", dial: "+7", mask: "+7 999 999 9999" },
    { name: "Ukraine", iso2: "ua", dial: "+380", mask: "+380 99 999 99 99" },
    { name: "Moldova", iso2: "md", dial: "+373", mask: "+373 999 999 99" },
    { name: "Armenia", iso2: "am", dial: "+374", mask: "+374 99 999 999" },
    { name: "Azerbaijan", iso2: "az", dial: "+994", mask: "+994 99 999 99 99" },
    { name: "Georgia", iso2: "ge", dial: "+995", mask: "+995 999 99 99 99" },
    { name: "Kyrgyzstan", iso2: "kg", dial: "+996", mask: "+996 999 999 999" },
    { name: "Tajikistan", iso2: "tj", dial: "+992", mask: "+992 99 999 99 99" },
    { name: "Turkmenistan", iso2: "tm", dial: "+993", mask: "+993 99 999 99 99" },
    { name: "Uzbekistan", iso2: "uz", dial: "+998", mask: "+998 99 999 99 99" }
];

// 2️⃣ Остальные страны с универсальной маской
const otherCountries = [
    { name: "USA", iso2: "us", dial: "+1", mask: "+999999999999999" },
    { name: "Germany", iso2: "de", dial: "+49", mask: "+999999999999999" },
    { name: "France", iso2: "fr", dial: "+33", mask: "+999999999999999" },
    { name: "Italy", iso2: "it", dial: "+39", mask: "+999999999999999" },
    { name: "Spain", iso2: "es", dial: "+34", mask: "+999999999999999" },
    { name: "UK", iso2: "gb", dial: "+44", mask: "+999999999999999" },
    { name: "Canada", iso2: "ca", dial: "+1", mask: "+999999999999999" },
    { name: "Australia", iso2: "au", dial: "+61", mask: "+999999999999999" },
    { name: "Brazil", iso2: "br", dial: "+55", mask: "+999999999999999" },
    { name: "Japan", iso2: "jp", dial: "+81", mask: "+999999999999999" },
    { name: "China", iso2: "cn", dial: "+86", mask: "+999999999999999" }
];

// 3️⃣ Объединяем страны
let allCountries = [];

fetch('https://raw.githubusercontent.com/vueform/international-phone-masks/main/masks.json')
    .then(res => res.json())
    .then(data => {
        allCountries = Object.entries(data).map(([code, country]) => ({
            iso2: code,        // ключ как ISO
            name: country.name,
...
function initPhoneInputs() {
    // Проверяем, что данные стран загружены
    if (!allCountries || allCountries.length === 0) {
        console.warn('Список стран не загружен');
        return;
    }
    ...
    document.querySelectorAll(".phone-wrapper").forEach(wrapper => {
        ...
        const countrySelect = wrapper.querySelector(".country-select");
        ...
        if (!countrySelect || !countryDropdown || !phoneInput) {
            console.warn('Не все элементы найдены в wrapper');
            return;
        }

        // Проверяем дочерние элементы
        const countryCodeEl = countrySelect.querySelector(".country-code");
        const flagEl = countrySelect.querySelector("span.fi");

        if (!countryCodeEl || !flagEl) {
            console.warn('Не найдены .country-code или span.fi');
            return;
        }

        let selectedCountry = allCountries.find(c => c.iso2.toLowerCase() === "ru") || allCountries[0];

        // создаем dropdown
        countryDropdown.innerHTML = '';
        allCountries.forEach(c => {
            const div = document.createElement("div");
            div.innerHTML = `<span class="fi fi-${c.iso2.toLowerCase()}"></span> ${c.name}`;
            div.dataset.code = c.iso2;
            div.dataset.dial = c.dial;
            div.dataset.mask = c.mask;
            countryDropdown.appendChild(div);

            div.addEventListener("click", () => {
                selectedCountry = c;
                flagEl.className = `fi fi-${c.iso2.toLowerCase()}`;
                countryCodeEl.textContent = c.name;
                applyMask(phoneInput, c);
                countryDropdown.style.display = "none";
            });
        });

        // применяем маску
        function applyMask(input, country) {
        ...
        // инициализация с первой страной
        flagEl.className = `fi fi-${selectedCountry.iso2.toLowerCase()}`;
        countryCodeEl.textContent = selectedCountry.name;
        applyMask(phoneInput, selectedCountry);
    ...
    // Закрытие dropdown при клике вне (один обработчик)
    document.addEventListener("click", function(e) {
        document.querySelectorAll(".country-dropdown").forEach(dropdown => {
            if (dropdown && dropdown.style) {
                const wrapper = dropdown.closest('.phone-wrapper');
                if (wrapper) {
                    const select = wrapper.querySelector('.country-select');
                    if (select && !select.contains(e.target) && !dropdown.contains(e.target)) {
...
let profileBlock = document.querySelectorAll('.profile__block');
let profileForm = document.querySelectorAll('.profile__form');

for (let index = 0; index < profileBlock.length; index++) {
    const element = profileBlock[index];
    element.querySelector('button').addEventListener('click', function(){
        element.classList.toggle('_active')
        profileForm[index].classList.toggle('_active')
    })
}

if(window.innerWidth < 700){
    document.querySelectorAll('.card__element').forEach(card => {

        const content = card.querySelector('.card__content');

        const fav = card.querySelector('.card__fav');
        const aktia = card.querySelector('.card__aktia');
        const price = card.querySelector('.card__price');
        const label = card.querySelector('.card__fav label');
        const last = card.querySelector('.card__last');
...
document.querySelectorAll('.card__quantity').forEach(qty => {
...
const lito = document.getElementById('lito');
const iurBlock = document.querySelector('.data-delivery__iur-lico');

if (lito && iurBlock) {
    const toggleIur = () => {
        iurBlock.classList.toggle('_active', lito.checked);
    };
    ...
    // слушаем изменение
    lito.addEventListener('change', toggleIur);
}
...
function init() {
    ...
    const points = [
        ...
        {
            coords: [55.761574, 37.603856],
            title: 'Склад',
            text: 'Склад №1'
        },
        {
            coords: [55.731574, 37.543856],
            title: 'Магазин',
            text: 'ТЦ Example'
        }
    ];
...
const toggle = document.querySelector('.profile__left-toggle');
const menu = document.querySelector('.profile__left-menu');
...
const dropZone = document.querySelector('.popup__drop');

// Проверяем, что dropZone существует
if (dropZone) {
    const fileInput = dropZone.querySelector('#file');
    const text = dropZone.querySelector('.popup__drop-text');

    // Проверяем, что fileInput и text существуют
    if (fileInput && text) {
        const MAX_FILES = 10;
        const ALLOWED_TYPES = ['image/png', 'image/jpeg', 'image/jpg'];
        let filesStore = [];

        // запрещаем дефолтное поведение
        ...
        function addFiles(newFiles) {
            ...
            filesArray.forEach(file => {
                if (filesStore.length < MAX_FILES) {
        ...
        function updateText() {
            if (!filesStore.length) return;
            text.innerHTML = filesStore
...
if (rating) {
    ...
    if (stars.length > 0 && input) {
        rating.addEventListener('click', (e) => {
            ...
            stars.forEach(s => {
                s.classList.toggle('_active', +s.dataset.value <= value);
            });
        });
        ...
        stars.forEach(star => {
            star.addEventListener('mouseenter', () => {
                ...
                stars.forEach(s =>
                    s.classList.toggle('_active', +s.dataset.value <= val)
                );
            });

            star.addEventListener('mouseleave', () => {
                const saved = +rating.dataset.rating;
                stars.forEach(s =>
                    s.classList.toggle('_active', +s.dataset.value <= saved)
                );
            });
        });
    }
