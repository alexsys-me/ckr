...
function initPhoneInputs() {
  document.querySelectorAll(".phone-wrapper").forEach(wrapper => {
    ...
    let selectedCountry = allCountries.find(c => c.iso2.toLowerCase() === "ru") 
                      || allCountries[0];
    ...
    function applyMask(input, country) {
      ...
      if (country.mask) {
        Inputmask({
		  ...
		  definitions: {
    '0': { validator: "[0-9]" } // теперь '0' означает любую цифру
  }
        }).mask(input);
      } else {
        // fallback для стран без маски
        Inputmask({
          mask: country.dial + " [999999999999]",
          greedy: false,
          placeholder: "_",
